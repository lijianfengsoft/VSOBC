...
csp_socket_t * csp_socket(uint32_t opts) {
	...
	if (opts & CSP_SO_RDPREQ) {
		csp_log_ODE_COM				5error("Attempt to create socket that requires RDP, but CSP was compiled without RDP support\r\n");
		return NULL;
	}
#endif
...
int csp_sendto(uint8_t prio, uint8_t dest, uint8_t dport, uint8_t src_port, uint32_t opts, csp_packet_t * packet, uint32_t timeout) {
				//(CSP_PRIO_NORM, CDH_NODE_BEACON_SERVER 10, CDH_PORT_BEACON_SERVER 30, 0, CSP_SO_NONE, packet, 0)
	packet->id.flags = 0;
