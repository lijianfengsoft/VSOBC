...
int csp_conn_init(void) {
	...
	srand(csp_get_ms());
	sport = (rand() % (CSP_ID_PORT_MAX - CSP_MAX_BIND_PORT)) + (CSP_MAX_BIND_PORT + 1);  //32~63
	...
	int i, prio;
	for (i = 0; i < CSP_CONN_MAX; i++) {
		for (prio = 0; prio < CSP_RX_QUEUES; prio++)//CSP_RX_QUEUES=4
...
csp_conn_t * csp_connect(uint8_t prio, uint8_t dest, uint8_t dport, uint32_t timeout, uint32_t opts) {
	...
	incoming_id.pri = prio;			//2
	incoming_id.dst = my_address;	//10
	incoming_id.src = dest;			//5
	incoming_id.sport = dport;		//1
	incoming_id.flags = 0;

	outgoing_id.pri = prio;			//2
	outgoing_id.dst = dest;			//5
	outgoing_id.src = my_address;	//10
	outgoing_id.dport = dport;		//1
	outgoing_id.flags = 0;
	...
	while (++sport != start)
	{
		...
		conn = csp_conn_find(incoming_id.ext, CSP_ID_DPORT_MASK); //0xFC000
