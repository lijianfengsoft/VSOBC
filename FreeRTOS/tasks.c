...
/*----------------------------------------------------------
*每个时钟节拍中断（滴答定时器中断）调用一次本次函数，增加时钟节拍计数器xTaskCount的值
*并且检查是否有任务需要取消阻塞
*xConstTickCount：增加计数，为0，也就是溢出时，就交换延时和溢出列表指针值
*/
...
BaseType_t xSwitchRequired = pdFALSE;
	...
	if( uxSchedulerSuspended == ( UBaseType_t ) pdFALSE )
	{
		...
		/*此处为修改处2018.1.13 by： LJF************************/
		if (xTickCount % 3600000 == 0)		//时钟节拍频率，周期是1ms,即1小时同步一次时间
		{
			timesync_nopara();
		}
		/******************************************************/
